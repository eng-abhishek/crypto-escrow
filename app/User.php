...
class User extends Authenticatable
{
     ...
     * @var array
     */
    public static $permissions = ['categories', 'messages', 'users', 'products', 'logs', 'disputes', 'tickets', 'vendorpurchase', 'purchases','announcements','featurepurchase','setting'];
    ...
    {
        $stubUser = new User();
        $stubUser -> username = config('app.name');
        return $stubUser;
    }
    ...
    public static function buyersWithPGPkey()
    {
        $allUsers = User::where('pgp_key','!=',null)->get();
        $onlyBuyers = $allUsers -> diff(Admin::allUsers());
        $onlyBuyers = $onlyBuyers -> diff(Vendor::allUsers());
    ...
    {
        return $this -> hasMany(\App\FeaturedProuctPurchase::class, 'user_id', 'id');
    }
    ...
    //     public function getLocalPriceFrom($price){

    //     return CurrencyConverter::convertToLocal($price);
    // }
}
