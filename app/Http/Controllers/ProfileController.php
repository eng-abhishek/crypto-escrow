...
class ProfileController extends Controller
{
    ...
    public function banned()
    {
        ...
        return view('frontend.profile.banned', [
            'until' => $until
        ]);
    }
    ...
    public function pgp()
    {
        ...
        return view('frontend.profile.pgp');
    }
    ...
    {
        $pgpKey = User::where('pgp_key',$request->newpgp)->count();
        
        if($pgpKey > 0){
         session() -> flash('errormessage', 'Oop`s this key has already userd by another user.');
    ...
    public function pgpConfirm()
    {
       // return view('profile.confirmpgp');
         return view('frontend.profile.confirmpgp');
    }
    ...
    public function oldpgp()
    {
        ...
        //     ]
        //     );
        return view('frontend.profile.oldpgp',
            [
                'keys' => auth() -> user() -> pgpKeys() -> orderByDesc('created_at') -> get(),
            ]
            );
    ...
    public function become()
    {
        ...
        $btcTousd = new BitcoinConverter();
        $btcToxmr = new MoneroConvert();
        
       // $vendorPurchases = VendorPurchase::where(['user_id'=>auth()->user()->id,'action_type'=>'vendor'])->get();

        $vendorPurchases = VendorPurchase::where(['user_id'=>auth()->user()->id])->get();
    
        return view('frontend.profile.become',[
                'vendorFee' => config('marketplace.vendor_fee'),
                'vendorFeeUSDToBTC' => round($btcTousd->usdToBtc(config('marketplace.vendor_fee')),5),
                'vendorFeeUSDToXMR' => round($btcToxmr->usdToXmr(config('marketplace.vendor_fee')),5),
                'depositAddresses' =>  $vendorPurchases,
    ...
    public function purchases($state = '')
    {
        ...
        //     'state' => $state,
        // ]);
             return view('frontend.profile.purchases.index', [
            'purchases' => $purchases,
            'state' => $state,
        ]);
    }
    ...
    public function purchase(Purchase $purchase)
    {
      ...
      if($purchase->coin_balance <= 0){

       $paymrntLastDay = Carbon::parse($purchase->created_at)->addMinute($time);
    ...
    {
        return view('frontend.profile.purchases.confirmdelivered', [
    ...
    {
        return view('frontend.profile.purchases.confirmcanceled', [
            'backRoute' => redirect() -> back() -> getTargetUrl(),
            'sale' => $purchase
        ]);
    }
...
//
    ...
    public function localcurrency(){
     return view('frontend.profile.local_currency');
    }

    public function twoFA(){
    return view('frontend.profile.2fa');
    }

    public function paymentAddress(){
    return view('frontend.profile.payment_address');
    }
