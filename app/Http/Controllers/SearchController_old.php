...
class SearchController extends Controller
{
    ...
    public function search(Request $request) {
       ...
       // $searchQuery = $request->search == null ? '' : strip_tags($request->search);

        $searchQuery = $request->search == null ? '' : strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $request->search));
        ...
        $query = route('search.show', [
            'query' => $searchQuery,
            'category' => $request->category,
            'type' => $request->product_type,
            'price_min' => strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $request->minimum_price)),
            'price_max' => strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $request->maximum_price)),
            'user' => strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $request->user)),
            'order_by' => $orderBy,
        ]);
         ...
         */
        //user
        $userQuery = strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $request->get('user')));
        if ($userQuery!== null){
        ...
        //price filter
        $minPriceQuery = strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $request->get('price_min')));
        $maxPriceQuery = strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $request->get('price_max')));
        ...
        return view('results', [
            ...
            'username' => strip_tags(preg_replace('#<script(.*?)>(.*?)</script>#is', '', $request->get('user'))),
        ]);
