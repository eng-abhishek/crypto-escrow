...
class DisplayProductsRequest extends FormRequest
{
    ...
    public function getProducts($status='') {
        ...
        $products = Product::with(['user','category','featured_product_purchase'])->where('active',$status); 
        }elseif(request()->segment(3)=='underreview'){

        $products = Product::with(['user','category','featured_product_purchase'])->where('active',0); 
        
        }else{
        
        $products = Product::with(['user','category','featured_product_purchase']);
        }
